...
**The hero shield carries no F number and is not on this list** (D-021). F1–F6 explain a
passage and sit beside it; the shield carries the title, the way the financial model case
study's beacon hero does. It is built by `planning/hero/build_hero.py` from the same four
committed Notion icon SVGs the F6 chips use, so the emblem and the chips cannot disagree.
...
| D-021 | **Hero image placement — the shield ships as the hero figure.** The shield graphic built under #184/#186 (asset-only until placement was ruled) sits inside the hero `<section>`, between the stat tiles and the `.docmeta` colophon — the slot the financial model case study's beacon hero occupies, so the two heroes read as one pattern. It takes **no F number** and is not a substitute for F1–F6: those explain a passage and sit beside it, this one carries the title. Consequences: the finished JPEG moves to `case-study-assets/img/` (the only deploying home; the planning copy was deleted rather than kept in parallel, and `build_hero.py`'s default output now points at the deployed path), the four icon hues ride in as figure content under D-015, and the coral ration in `case-study.css` is untouched because nothing in a JPEG is drawn by the stylesheet. The build was verified byte-identical on re-run before the embed landed. The tiled print variant stays under `planning/` and does not deploy. **Still open:** the social card (`og-wolfpack-ai-command.png`) is briefed against F1 as its inset and remains unbuilt — the shield is now the obvious alternative inset, and that is Ry's call, not a consequence of this one | Ry, 2026-08-15 |

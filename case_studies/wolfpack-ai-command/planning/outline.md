...
### Part 1 — Hero
- Title: **editorial and argument-led, not the product name** (D-011) — like the other
  two case studies' titles, in the territory of *The Role, Re-Divided*; the exact title
  is Ry's pick (open). The name **Wolfpack AI Command** appears in the
  standfirst/subtitle instead — its first rationed use. Subtitle territory: *the
  project-management role, re-divided between humans and supervised AI.*
- Standfirst names the split thesis in two sentences: the PM job is two jobs braided
  together; this system unbraids them.

/* ==========================================================================
   Portfolio page — scroll reveal + scroll-spy. No dependencies.

   Copied byte-identical from hire/assets/js/reveal.js apart from this header
   comment, per the same rule case_studies/README.md states: reveal timing and
   scroll-spy are supposed to match every other long-form page in this repo, and
   a rewrite is how two pages quietly stop matching. Do not edit the code below
   here — edit it in one place and re-copy.

   Two non-negotiables, both load-bearing:

   1. The `.js` class is added HERE, and the hidden initial state in
      portfolio.css is scoped to it. With JavaScript off, broken, or blocked,
      every element renders visible and the page is complete. A portfolio that
      requires JS to show the work is a broken portfolio — the same failure mode
      as the v2.1 image-only résumé header
      (resume_design/header-footer-spec.md §9).
...
(function () {
  ...
  var revealables = document.querySelectorAll('.reveal, .tl__row');

...
| R18 | Q3 portfolio + case-study buttons (added 2026-09-06) | **A Q3-page ruling, recorded here** because this spec is where the two pages' relationship lives — the contract carries it as delta D16. Upstream, the dead portfolio placeholder became a **live navy-ghost button** to `https://intake.wolfstrategyllc.com/portfolio/`, and two application tiles gained a navy-ghost `btn--sm` *"Read the case study"* link placed **after** `.app__blurb`. **Ghost, not coral — and the Q3 coral ration dropped 9 → 8** as a result, enumerated in that page's `css/rates.css` header comment. ⚠️ **This ruling's closing sentence was corrected on 2026-09-08 (#299).** It read: *"Coral counts only ever go down, on both pages; a ration that moves back up is a regression, not a ruling."* The first half is the **default** and still governs a session's own judgment — but the second half was too strong and R19 is the counter-example: a ration **may** move up when **Ry asks for a named placement**, dated in the sheet and in `docs/site-brief.md` §2. What has not changed is that a session may never add a coral use on its own initiative. This correction is about who may authorize an increase, not about loosening the ration. **Nothing ports here**: D9 bans the application-screenshot grid on this page, so there is no grid for these buttons to sit in — this page reaches the portfolio through its own closing `#work` section (R14/D15), unchanged. |
| R19 | Pilot section rendered as three blocks (added 2026-09-08) | The `#pilot` section renders its six points as **three `.pblock` cards** in a `.pblocks` grid — **Kickoff & rate** ($500 / KICKOFF FEE, points 1–3) · **Recommended pace** (10–20 / HOURS PER WEEK, points 4–5) · **After the 30 days** (point 6) — echoing the `.tier` cards below rather than the `.rail` panel it replaced. **NO COPY CHANGED**: all six points are verbatim, and block 3's heading is point 6's own bold lead-in promoted out of the list with the remainder of that sentence as its body. The only strings added are the three mono kickers and block 2's sub-label. `.pblock__head` carries a `min-height` so the divider rule lands at the same height in all three cards — **re-measure it if the figure size, kicker or sub-label changes.** The CTA stays **section-level, below the grid**, and stays navy-ghost (R15). ⚠️ **PUBLIC-PAGE-ONLY — contract D17.** The Q3 page keeps the single panel; this is the first layout divergence between the pair, it is deliberate, and the six facts still mirror exactly. **Coral:** the kickoff card takes the border + raised surface `.tier--featured` uses — Ry authorized this one placement, and it is the **first increase on this sheet's ration** (7 → 8 by the brief's count, 8 → 9 by the sheet's own enumeration; ruling X-3's original discrepancy is carried forward, not resolved). See `docs/site-brief.md` §2 and the corrected note in R18. |
...
**Since 2026-09-08 (#299) those six points are laid out as THREE CARDS, not one panel** —
`.pblocks` holding three `.pblock` articles, grouped **1–3 / 4–5 / 6**, each with a mono kicker
and a figure or lead: **Kickoff & rate** ($500 / KICKOFF FEE) · **Recommended pace** (10–20 /
HOURS PER WEEK) · **After the 30 days** (*"Day 31 is an open door."*). The kickoff card is
coral-bordered; the button sits **below the grid**, still section-level and still navy-ghost.
The points themselves are **unchanged and verbatim** — see R19, and contract **D17** for why
the Q3 page deliberately still shows the single panel.

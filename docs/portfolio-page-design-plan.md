...
| **D-003** | **No links to the `/hire/` pages** | Those are `noindex`, direct-link only, deliberately linked from nothing. A public indexed page linking them defeats that in one step. Hiring managers get the resume from Ry directly. Ruling: Ry, 2026-08-04. |
...
| **D-008** | Case studies section ships with **one live card and one in-preparation card** | Only the ops financial model case study is published. The SetMaster 3 card uses the same `.case__chip` in-preparation device the `hire/` pages use, so adding the link later changes one element, not the layout. |

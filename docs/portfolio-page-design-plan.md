...
| **D-010** | **Linking the SetMaster 3 case study is fine, and the card is a live link** | The card was briefly held back on the theory that D-003 extended to any `noindex`, direct-link-only page, and the SetMaster 3 case study is one. **That was wrong** (Ry, 2026-08-04). Three checks settle it: the case study contains **no link to `hire/`** — the only `hire/` string in that file is an HTML comment — so linking it creates no path to the résumés; it is a **case study, not a résumé**, so none of D-003's actual harm applies; and its own `noindex` protects a **different pair entirely**, the case study against the planned SetMaster 3 product landing page at `/setmaster3/`, which has nothing to do with `hire/`. A public blog post (#103) already links it, so this is not new exposure either. |

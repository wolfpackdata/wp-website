...
| D-012 | **Two recruiter-brief pages join `hire/`** — `recruiter-brief/` and `recruiter-brief-music/`, rendering `ryan-resume-dev/for_recruiters/*.md` on this design system. Ry, 2026-08-20 (#246). Audience is agency recruiters and staffing desks, not hiring managers. Same `noindex, nofollow` as D-002: a comp band and an availability claim are for the recruiter Ry emailed, not for search. | Same shared `assets/`, same `hire.css` (three additions at the end of the sheet — `.matrix--3`, `.terms`/`.term`, `.linkgrid` — no per-page stylesheet), same nav / hero slab / closing block / coda / footer, so the four pages read as one authored set. The briefs link the résumé pages relatively (`../ryan-hickey/`, `../ryan-hickey-music/`): hire → hire is inside the one deploy unit D-003's prohibition protects. `check_meta.py` has rows for both. **Built locally end to end; the intake deploy waits for Ry.** |
| D-013 | **The briefs carry the 200×200 logo on `twitter:card summary`, not a built card.** Flagged for Ry, not settled. | D-015/D-016 of the social-cards plan upgraded the résumé pages because a pasted résumé URL *is* the first impression. A brief is linked from an email that already carries the résumé, so its preview is context rather than the opening. If Ry rules otherwise, the upgrade is one `build_cards.py` run and a head edit — never a hand-made image. |
| D-014 | **Brief content is the markdown's, not the YAML's, and `for_recruiters/verify_brief.py` guards the two copies.** | The pitch must be verbatim; every dollar figure, title, and excluded industry in the markdown must appear on the page, and each check asserts it found its anchor first. The pages are deliberately **not** in `verify_facts.py` `HTML_PAGES` and use no `.app__name` / `.app__blurb`: the proof points are the brief's own wording, so check 6 would have nothing to guard and would fail loudly on zero matches. Same status `pilot-project/`'s fresh-written blurbs have. |
| D-015 | **Two outbound destinations the résumé pages don't carry: the public rate card and the pilot page**, as inline navy links inside the secondary-ask card. | The brief's contract ask quotes the rate card's numbers and the pilot's fee; a recruiter should be able to open both. Plain inline text, not buttons, so the Contact CTA is undisplaced. `pilot-project/` is `noindex` and so are the briefs — the harm its link rule names (anchoring a *public* reader at the cheapest price Wolfpack sells) does not arise on a page only a recruiter is sent. |
